...
#
# Another hack below is the "slib-override" directory.
...
# So, the slib-override directory is put at the top of the search order
...
# slib-override directory tree is removed.
define prepare_loader_cache
    ...
    if [[ -n "$(TARGET64)" ]]; then \
        if [[ $1 -eq 1 ]]; then \
            echo "$(AT_DEST)/slib-override"   >  "$(DYNAMIC_LOAD)/at64.ld.conf"; \
        fi; \
    ...
    if [[ $1 -eq 2 ]]; then \
        rm -rf $(AT_DEST)/slib-override; \
    else \
        for cpu in $(BUILD_ACTIVE_MULTILIBS); do \
            mkdir -p $(AT_DEST)/slib-override/"$${cpu}"; \
            (\cd $(AT_DEST)/lib64 && \
             find . -maxdepth 1 -type f \
                  -exec cp -p '{}' $(AT_DEST)/slib-override/"$${cpu}"/'{}' \; \
            ); \
        done; \
    fi; \
